...
def index(request):
    ...
    mona = {'allProds': allProds}
    return render(request, 'shop/index.html', mona)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    mona = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        mona = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', mona)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            mona_dict['CHECKSUMHASH'] = Checksum.generate_checksum(mona_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'mona_dict': mona_dict})
